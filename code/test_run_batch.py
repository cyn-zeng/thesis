...
from prompts import s1_conditions
...
OUT_PATH = "output/test_outputs.csv"
...
with open(OUT_PATH, "a", encoding="utf-8", newline="") as f:
    fieldnames = ["ts", "model", "study_condition", "input", "output", "id"]
    ...
    for MODEL in free_models:
        for i, (key, prompt) in enumerate(s1.items(), start=1):
            ...
            print(f"[{i}/{len(s1)}] saved {len(text)} question-answer pairs for model {MODEL} and condition {key}")
            time.sleep(0.1)
