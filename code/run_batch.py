...
from models import free_model, study_models
...
with open(OUT_PATH, "a", encoding="utf-8", newline="") as f:
    fieldnames = ["ts", "model", "input", "output", "id"]
    ...
    for MODEL in study_models:
        for i, prompt in enumerate(s1_b_prompts, start=1):
            ...
            print(f"[{i}/{len(s1_b_prompts)}] saved {len(text)} chars")
            time.sleep(0.1)
